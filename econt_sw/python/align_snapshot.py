...
#i2c.write(0x20, 0x0380, [0b00000111])
i2c.write(0x20, 0x0380, [0b00000010])
...
with numpy.printoptions(formatter={'int':lambda x: f'{x:08x}'}, linewidth=120):
    for i in range(12):
        logging.debug(f'Chan Aligner {i:02d} sel 0x{sel[i]:02x}, status 0x{status[i]:02x}, snapshot 0x{snapshot[i]:48x}')
