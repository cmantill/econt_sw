...
# default for 3 (78,54,25)
# default for 2 (52,28,25)
Buff_T1 = 52
Buff_T2 = 28
Buff_T3 = 25
i2c.write(0x20, 0x03a9 + 0x02, Buff_T1.to_bytes(2, 'little'))
...
outdata = numpy.array([[numpy.copy(lc_mem[j]) for i in range(FIFO_occupancy[j])] for j in range(TX_en)]).T

import pandas as pd
file_name = "out/Buffer_TS_nTX%i_T1-%i_T2-%i_T3-%i.csv"%(TX_en,Buff_T1,Buff_T2,Buff_T3)
df = pd.DataFrame(outdata)
df.to_csv(file_name,header=False,index=False)

vHex = numpy.vectorize(hex)
df = vHex(df)
pd.DataFrame(df).to_csv(file_name.replace('.csv','_HEX.csv'),header=False,index=None)

# print in the same format
data_str = []
for idata,dt in enumerate(outdata):
    data_str.append(['{0:08X}'.format(int(outdata[idata][il])) for il,elink in enumerate(outdata[0])])
